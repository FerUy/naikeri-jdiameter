...
public class NetworkGuard implements INetworkGuard {
  ...
  public void onNewRemoteConnection(Server globalServer, Association association, SCTPServerConnection connection) {
    ...
    try {
      ...
      InetAddress remoteInetAddress = InetAddress.getByAddress(peerAddress, InetAddress.getByName(peerAddress).getAddress());
      InetAddress localInetAddress = InetAddress.getByAddress(localAddress, InetAddress.getByName(localAddress).getAddress());
      SCTPServerConnection remoteClientConnection = new SCTPServerConnection(null, remoteInetAddress,
          peerPort, localInetAddress, localPort, parser, null, this, globalServer, association, connection.getManagement());
