...
public class SCTPTransportServer {
  ...
  private Integer maxConcurrentConnectionCount;
    ...
    try {
      ...
      if (server == null) {
        server = this.management.addServer(serverName, origAddress.getAddress().getHostAddress(), origAddress.getPort(),
            IpChannelType.SCTP, true, this.maxConcurrentConnectionCount, extraHostAddress);
      }
  ...
  public void setMaxConcurrentConnectionCount(Integer maxConcurrentConnectionCount) {
  ...
  public void setDestAddress(InetSocketAddress address) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Destination address is set to [{}:{}]", destAddress.getAddress(), destAddress.getPort());
    }
  }
  ...
  public void setOrigAddress(InetSocketAddress address) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Origin address is set to [{}:{}]", origAddress.getAddress(), origAddress.getPort());
    }
  }
