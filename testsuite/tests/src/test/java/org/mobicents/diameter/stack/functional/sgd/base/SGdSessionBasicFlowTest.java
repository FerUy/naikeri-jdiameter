...
public class SGdSessionBasicFlowTest {
  ...
  public SGdSessionBasicFlowTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  @Test
  public void testMOForwardShortMessage() {
    try {
      // pain of parameter tests :) ?
      serverNode1.sendMOForwardShortMessageRequest();
      waitForMessage();

      clientNode.sendMOForwardShortMessageAnswer();
      waitForMessage();

      if (!clientNode.isReceivedOFR()) {
        fail("Did not receive OFR! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
      }

      if (!serverNode1.isReceivedOFA()) {
        fail("Did not receive OFA! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
      }
  ...
  public static Collection<Object[]> data() {
    ...
    Class<SGdSessionBasicFlowTest> t = SGdSessionBasicFlowTest.class;
