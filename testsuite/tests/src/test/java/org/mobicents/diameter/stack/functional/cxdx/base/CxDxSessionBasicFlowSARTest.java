...
public class CxDxSessionBasicFlowSARTest {
  private ClientSAR clientNode;
  private ServerSAR serverNode1;
  private final URI clientConfigURI;
  private final URI serverNode1ConfigURI;

  public CxDxSessionBasicFlowSARTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void setUp() throws Exception {
    try {
      this.clientNode = new ClientSAR();
      this.serverNode1 = new ServerSAR();
  ...
  @Test
  public void testServerAssignment() {
    try {
      // pain of parameter tests :) ?
      clientNode.sendServerAssignment();
      waitForMessage();

      serverNode1.sendServerAssignment();
      waitForMessage();
    }
    catch (Exception e) {
      e.printStackTrace();
    ...
    if (!serverNode1.isReceivedServerAssignment()) {
      fail("Did not receive SAR! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }

    if (!clientNode.isReceivedServerAssignment()) {
      fail("Did not receive SAA! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }
