...
public class ShSessionFlowWithAllAVPsTest {
  ...
  public ShSessionFlowWithAllAVPsTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void testMultipleMessage() {
    try {
      ...
      clientNode.sendProfileUpdateRequest();
      waitForMessage();

      serverNode1.sendProfileUpdateAnswer();
      waitForMessage();

      clientNode.sendSubscribeNotificationsRequest();
      waitForMessage();

      serverNode1.sendSubscribeNotificationsAnswer();
      waitForMessage();

      serverNode1.sendPushNotificationRequest();
      waitForMessage();

      clientNode.sendPushNotificationAnswer();
      waitForMessage();
    ...
    if (!serverNode1.isReceivedUDR()) {
      fail("Did not receive UDR! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }

    if (!clientNode.isReceivedUDA()) {
      fail("Did not receive UDA! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }
    ...
    if (!serverNode1.isReceivedSNR()) {
      fail("Did not receive SNR! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }

    if (!clientNode.isReceivedSNA()) {
      fail("Did not receive SNA! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }

    if (!clientNode.isReceivedPNR()) {
      fail("Did not receive PNR! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }

    if (!serverNode1.isReceivedPNA()) {
      fail("Did not receive PNA! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }
  ...
  public static Collection<Object[]> data() {
    ...
    Class<ShSessionFlowWithAllAVPsTest> t = ShSessionFlowWithAllAVPsTest.class;
