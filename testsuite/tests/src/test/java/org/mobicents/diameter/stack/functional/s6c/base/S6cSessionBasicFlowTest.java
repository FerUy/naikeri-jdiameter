...
public class S6cSessionBasicFlowTest {
  // TODO: add test on replicated nodes ?
  private ClientS6c clientNode;
  private ServerS6c serverNode1;
  private final URI clientConfigURI;
  private final URI serverNode1ConfigURI;

  public S6cSessionBasicFlowTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void setUp() throws Exception {
    try {
      this.clientNode = new ClientS6c();
      this.serverNode1 = new ServerS6c();
  ...
  @Test
  public void testSendRoutingInfoForSM() {
    try {
      // pain of parameter tests :) ?
      clientNode.sendSendRoutingInfoForSMRequest();
      waitForMessage();

      serverNode1.sendSendRoutingInfoForSMAnswer();
      waitForMessage();
    ...
    if (!serverNode1.isReceivedSRR()) {
      fail("Did not receive SRR! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }

    if (!clientNode.isReceivedSRA()) {
      fail("Did not receive SRA! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }
  ...
  @Test
  public void testReportSMDeliveryStatus() {
    try {
      // pain of parameter tests :) ?
      clientNode.sendReportSMDeliveryStatusRequest();
      waitForMessage();

      serverNode1.sendReportSMDeliveryStatusAnswer();
      waitForMessage();
    ...
    if (!serverNode1.isReceivedRDR()) {
      fail("Did not receive RDR! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }

    if (!clientNode.isReceivedRDA()) {
      fail("Did not receive RDA! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }
  ...
  @Test
  public void testAlertServiceCentre() {
    try {
      // pain of parameter tests :) ?
      serverNode1.sendAlertServiceCentreRequest();
      waitForMessage();

      clientNode.sendAlertServiceCentreAnswer();
      waitForMessage();
    ...
    if (!clientNode.isReceivedALR()) {
      fail("Did not receive ALR! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }

    if (!serverNode1.isReceivedALA()) {
      fail("Did not receive ALA! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }
  ...
  public static Collection<Object[]> data() {
    ...
    String client = "configurations/functional-s6c/config-client.xml";
    String server1 = "configurations/functional-s6c/config-server-node1.xml";

    //String replicatedClient = "configurations/functional-s6c/replicated-config-client.xml";
    //String replicatedServer1 = "configurations/functional-s6c/replicated-config-server-node1.xml";

    Class<S6cSessionBasicFlowTest> t = S6cSessionBasicFlowTest.class;
