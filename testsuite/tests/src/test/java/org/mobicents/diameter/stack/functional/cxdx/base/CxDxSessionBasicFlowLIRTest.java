...
public class CxDxSessionBasicFlowLIRTest {
  private ClientLIR clientNode;
  private ServerLIR serverNode1;
  private final URI clientConfigURI;
  private final URI serverNode1ConfigURI;

  public CxDxSessionBasicFlowLIRTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void setUp() throws Exception {
    try {
      this.clientNode = new ClientLIR();
      this.serverNode1 = new ServerLIR();
  ...
  public void testLocationInfo() {
    try {
      ...
      clientNode.sendLocationInfo();
      waitForMessage();

      serverNode1.sendLocationInfo();
      waitForMessage();
    }
    catch (Exception e) {
      e.printStackTrace();
    ...
    if (!serverNode1.isReceivedLocationInfo()) {
      fail("Did not receive LIR! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }

    if (!clientNode.isReceivedLocationInfo()) {
      fail("Did not receive LIA! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }
