...
public class CxDxSessionBasicFlowMARTest {
  private ClientMAR clientNode;
  private ServerMAR serverNode1;
  private final URI clientConfigURI;
  private final URI serverNode1ConfigURI;

  public CxDxSessionBasicFlowMARTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void setUp() throws Exception {
    try {
      this.clientNode = new ClientMAR();
      this.serverNode1 = new ServerMAR();
  ...
  @Test
  public void testMultimediaAuth() {
    try {
      // pain of parameter tests :) ?
      clientNode.sendMultimediaAuth();
      waitForMessage();

      serverNode1.sendMultimediaAuth();
      waitForMessage();
    ...
    if (!serverNode1.isReceivedMultimediaAuth()) {
      fail("Did not receive MAR! " + "Server ER:\n" + serverNode1.createErrorReport(this.serverNode1.getErrors()));
    }

    if (!clientNode.isReceivedMultimediaAuth()) {
      fail("Did not receive MAA! " + "Client ER:\n" + clientNode.createErrorReport(this.clientNode.getErrors()));
    }
  ...
  public static Collection<Object[]> data() {
    ...
    Class<CxDxSessionBasicFlowMARTest> t = CxDxSessionBasicFlowMARTest.class;
